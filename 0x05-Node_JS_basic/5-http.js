...
function countStudents(path, stream) {
  if (fs.existsSync(path)) {
    ...
    result.shift();
    const lines = [];
    result.forEach((data) => lines.push([data[0], data[3]]));
    const fields = new Set();
    lines.forEach((item) => fields.add(item[1]));
    const final = {};
    fields.forEach((data) => { (final[data] = 0); });
    lines.forEach((data) => { (final[data[1]] += 1); });
    stream.write(`Number of students: ${result.length}\n`);
    const temp = [];
    Object.keys(final).forEach((data) => temp.push(`Number of students in ${data}: ${final[data]}. List: ${lines.filter((n) => n[1] === data).map((n) => n[0]).join(', ')}\n`));
